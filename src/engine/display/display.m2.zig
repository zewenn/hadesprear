...
pub fn update() !void {
    ...
    std.log.info("Got Entities", .{});
    defer {
        std.log.info("Freed entity slice", .{});
        ecs.alloc.free(entities);
    }
    std.sort.insertion(*ecs.Entity, entities, {}, sortEntities);
    std.log.info("Got Entities Sorted", .{});

    for (entities, 0..) |e, i| {
        std.log.info("In list: {s} ({d}/{d})", .{ e.id, i + 1, entities.len });
    }

    SyncPrevious: {
        var prev_kIt = previous_map.keyIterator();

        while (prev_kIt.next()) |prev_id| {
            const k2 = prev_id.*;

            if (!ecs.existsEntity(prev_id.*)) {
                _ = previous_map.remove(prev_id.*);
                alloc.free(k2);
            }
        }
    ...
    for (entities, 0..) |entity, i| {
        std.log.info("Processing entity: {s} ({d}/{d})", .{ entity.id, i + 1, entities.len });
        ...
        std.log.info("\t[{d}/{d}] Got transform", .{ i + 1, entities.len });

        if (entity.get(ecs.components.Display, "display")) |_display| {
            display = _display.*;
        } else continue;
        std.log.info("\t[{d}/{d}] Got display", .{ i + 1, entities.len });

        const use_previous: bool = Decide: {
            if (!previous_map.contains(entity.id)) {
                std.log.info("\t[{d}/{d}] Isn't in previous map", .{ i + 1, entities.len });

                const entry = previous_map.getOrPut(try alloc.dupe(u8, entity.id)) catch {
                    std.log.debug("failed to store entity", .{});
                    break :Decide false;
                };

                std.log.info("\t[{d}/{d}] Created key value pair", .{ i + 1, entities.len });
                ...
                std.log.info("\t[{d}/{d}] Is now in previous map", .{ i + 1, entities.len });
                break :Decide false;
            }
        ...
        std.log.info("\t[{d}/{d}] Decided to use previous or not ({s})", .{ i + 1, entities.len, if (use_previous) "true" else "false" });
    ...
    std.log.info("Finished drawing entities!", .{});
